...
\label{anhangC}
Wie schon im Kapitel 4.3.2 angedeutet, verwendet man zur Übersetzung der Constraints in MCP-FD-Modellierungssprache unterschiedliche Implementierungen für die beiden Constraint-Solver. Genauer gesagt, die Implementierung variiert für die beiden Solver hinsichtlich der Übersetzung von FD-Listen in MCP \emph{collections}. Der Gecode-Solver erwartet, dass man für jede FD-Liste eine MCP-\emph{collection}-Variable einführt und etwaigen konstanten Werte in dieser Liste mit Hilfe der \lstinline|(@!!)|-Funktion des \lstinline|(@=)|-Constraints eine Position in der durch die Variable repräsentierten \emph{collection} zuweist (vergleiche hierzu die Funktion \lstinline|newColCs|). Der Overton-Solver kann hingegen auch mit konstanten MCP-\emph{collections}, also mit Listen vom Typ \lstinline|ModelInt| arbeiten.
\\
Um neue MCP-\emph{collection}-Variablen für den Gecode-Solver einführen zu können, ist es notwendig, FD-Listen eindeutig identifizieren zu können. Nur auf diese Weise kann gewährleistet werden, dass bei der Übersetzung die gleiche FD-Liste auf die gleiche MCP-\emph{collection}-Variable abgebildet wird. Daher wird der unten angegebene Datentyp zur Repräsentation von FD-Listen eingeführt. Für diese FD-Listen werden dann bei der Übersetzung nach dem gleichen Prinzip, dass auch schon zur Übersetzung von FD-Variablen angewandt wurde, neue MCP-\emph{collection}-Variablen eingeführt (siehe dazu die Funktionen \lstinline|translateGecodeList| und \lstinline|newColVar|).

\begin{lstlisting}[language=Haskell,basicstyle=\footnotesize\ttfamily,caption=Einführung eines Identifiers für FD-Listen]
-- representation of list of fd variables
...
\begin{lstlisting}[language=Haskell,basicstyle=\footnotesize\ttfamily,frame=no]
...
-- describing their size and elements (only used for translateGecode)
