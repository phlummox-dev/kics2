...
\begin{lstlisting}[language=Haskell,caption=Beispiel: Datentypdeklarationen]
...
\begin{lstlisting}[language=Haskell,caption=Beispiel: Integerbaum]
...
\begin{lstlisting}[language=Haskell,caption=Beispiele für Typsynonyme in Curry]
...
\begin{lstlisting}[language=Haskell,caption=Beispiel: Fakultätsfunktion]
fac :: Int -> Int
...
\begin{lstlisting}[language=Haskell,caption=Definition der Fakultätsfunktion mit Pattern Matching]
facPM :: Int -> Int
facPM 0 = 1
facPM n = n * facPM (n-1)
\end{lstlisting}
...
\begin{lstlisting}[language=Haskell,caption=Beispiele für nicht-deterministische Funktionen]
(?) :: a -> a -> a
x ? _ = x
_ ? y = y
...
\begin{lstlisting}[language=Haskell,caption=Curry-Gleichheitsconstraint]
...
\begin{lstlisting}[language=Haskell,frame=no,caption=Beispiel: Pfade in einem gerichteten Graph]
data Node = A | B | C | D | E
...
\begin{lstlisting}[language=Haskell,caption=Beipiel: N-Damen-Problem mit Curry (PAKCS)]
queens options n l =
       gen_vars n =:= l &
       domain l 1 (length l) &
       all_safe l &
...
\par
Im Allgemeinen lässt sich ein Finite Domain Constraint Problem in Curry (PAKCS) also wie folgt spezifizieren (die Reihenfolge der Schritte 1. bis 3. sollte dabei eingehalten werden):
\begin{enumerate}
\item Festlegen des Wertebereichs der Constraint-Variablen mit dem \lstinline|domain|-Constraint
\item Modellieren des Problems durch Angabe weiterer Constraints (wie \lstinline|(=#)|, \lstinline|(/=#)|, \lstinline|(<#)|, \lstinline|(allDifferent)| etc.) über den Constraint-Variablen
...
\begin{lstlisting}[language=Haskell,caption=Beispiel: \lstinline|xorSelf|]
xorSelf :: Bool -> Bool
xorSelf x = xor x x
...
\begin{lstlisting}[language=Haskell,caption=Propagieren des Identifikators]
...
\begin{lstlisting}[language=Haskell,caption=Beispiel: Nicht-Deterministischer Generator für boolesche Listen]
...
\begin{lstlisting}[language=Haskell,caption=Definition von Bindungsconstraints und -entscheidungen]
...
Somit ist ein Term eine Constraint-Variable des entsprechenden Typs, welche durch eine \lstinline|Int|-Referenz identifiziert wird. Mit den Term-Typen und den oben vorgestellen Ausdruckstypen werden dann die eigentlichen Typen zur Modellierung von Constraint-Problemen eingeführt: \lstinline|ModelInt| für arithmetische Integer-Ausdrücke, \lstinline|ModelCol| für Listen über Integer-Ausdrücken (auch als \emph{collections} bezeichnet) und \lstinline|Model| für Finite Domain Constraints.

\begin{lstlisting}[language=Haskell,caption=Typen für Integer- und Listenausdrücke sowie FD-Constraints im MCP-Framework]
...
\\
Neben der allgemeinen Typklasse für Solver gibt es im MCP-Framework auch eine Typklasse speziell für FD-Solver (vollständige Angabe im Anhang):
...
class (Solver s, Term s (FDIntTerm s), Term s (FDBoolTerm s)) => 
  FDSolver s where
    ...
    ...
\end{lstlisting}
Diese Typklasse macht wie auch schon die MCP-Solver-Typklasse Gebrauch von der Spracherweiterung der \emph{TypeFamilies}. So muss man bei der Instanziierung dieser Typklasse für einen bestimmten FDSolver die Typen für Integerterme und boolesche Terme angeben, die dieser FD-Solver verwendet. Des Weiteren müssen Typen für Integer-, Integerarray- und boolesche Ausdrücke definiert werden. Die Typklasse stellt außerdem eine Reihe von Funktionen zur Implementierung bereit, darunter unter anderem Konstruktorfunktionen für die zuvor definierten Ausdruckstypen und Funktionen zum Hinzufügen von Gleichheitsconstraints.
\\
...
\end{lstlisting}
Dieser Typ wird im MCP-Framework zu einer Instanz der \lstinline|Solver|-Typklasse gemacht. Als Constraints legt man dabei die FD-Constraints der MCP-Modellierungssprache (\lstinline|Model|) fest.
...
\begin{lstlisting}[language=Haskell,caption=Modellierung des N-Damen-Problems mit dem MCP-Framework]
...
queens n = exists $ \p -> do
  ...
  return p
\end{lstlisting} 
Ein MCP-Modell beginnt üblicherweise mit dem Aufruf von \lstinline|exists|. Diese Funktion erwartet eine Funktion, die als Argument einen Solver-Term (hier die \emph{collection} \lstinline|p|) entgegennimmt und ein neues Baummodell liefert. Die Größe der MCP \emph{collection} wird dann durch das \lstinline|size|- und das \lstinline|(=#)|-Constraint auf die übergebene Problemgröße \lstinline|n| festgelegt. Das \lstinline|allin|-Constraint gibt den Wertebereich für alle Constraint-Variablen in der \emph{collection} vor. Mit Hilfe zweier Schleifen und des selbst definierten \lstinline|noattack|-Constraints werden alle notwendigen Beschränkungen erzeugt, so dass keine Dame mit einer anderen in der gleiche Reihe oder Diagonale des Schachbretts steht. Abschließend wird ein \lstinline|Return|-Knoten mit der \emph{collection} \lstinline|p| erzeugt. Über den in diesem Knoten angegebenen Constraint-Variablen führen die FD-Solver beim Lösen des Modells das Labeling durch.
\\
Bei der Modellierung kommen außerdem zwei Hilfsfunktionen zum Einsatz: Mit \lstinline[breaklines=true]|cte :: Integral a => a -> ModelInt| kann man ganze Zahlen in einen MCP-Integerausdruck umwandeln. Und mit \lstinline|(!) :: ColExpr t c b -> Expr t c b -> Expr t c b| kann man ein bestimmtes Feld einer MCP \emph{collection} zugreifen.
\par
