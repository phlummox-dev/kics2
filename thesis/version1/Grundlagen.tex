...
\subsection{Curry}
\label{section_curry}
Der folgende Abschnitt liefert eine kurze Einführung in die funktional-logische Programmiersprache Curry. Dabei werden nur die wichtigsten und für das Verständnis dieser Arbeit relevanten Features dieser Sprache vorgestellt. Eine ausführlichere Einführung findet man unter \textcolor{red}{$<$hier Verweis auf Quellen$>$}
\\
\\
...
data t = !$C_{1}$! !$t_{11}$! ... !$a_{1n_{1}}$! | ... | !$C_{k}$! !$t_{k1}$! ... !$a_{kn_{k}}$!
\end{lstlisting}
Mit der obigen Deklaration wird ein neuer Datentyp $t$ mit $k$ unterschiedlichen Konstruktoren $C_{1}, ..., C_{k}$ definiert. Die $t_{i1_{i}}, ..., t_{in_{i}}$ sind dabei mögliche Argumenttypen der Konstruktoren, das heißt es gilt: $C_{i} :: t_{i1_{i}} \rightarrow ... \rightarrow t_{in_{i}} \rightarrow t$ für alle $i \in{\{1,...,k\}}$.
\\
Neben diesen einfachen Datentypdeklarationen bietet Curry auch die Möglichkeit polymorphe Datenstrukturen zu definieren:
\begin{lstlisting}[language=Haskell,escapechar=!]
data t !$a_{1}$! ... !$a_{n}$! = !$C_{1}$! !$t_{11}$! ... !$a_{1n_{1}}$!
                | ...
                | !$C_{k}$! !$t_{k1}$! ... !$a_{kn_{k}}$!
\end{lstlisting}
Zur Definition von polymorphen Datenstrukturen verwendet man sogenannte Typkonstruktoren. In der obigen Deklaration ist $t$ ein solcher Typkonstruktor mit $n$ Typvariablen $a_{1}, ..., a_{n}$. Zur Konstruktion eines neuen Typs werden diese Typvariablen mit konkreten Typen belegt.
\\
...
simpleIntTree = Branch (Branch Empty 2 Empty) 1 (Branch Empty 3 Empty)
\end{lstlisting}
Mit dem Schlüsselwort \textbf{type} können wie in Haskell Typsynonyme definiert werden. Damit hat man zum einen die Möglichkeit komplexe Typbezeichnungen durch einfachere zu ersetzen und so den Code lesbarer zu gestalten, zum anderen kann man aber auch einfachen Typen einen anderen Namen zuordnen, um auf diese Art die Verständlichkeit zu erhöhen, wie die folgenden Beispiele zeigen:
\begin{lstlisting}[language=Haskell]
...
f !$p_{1}$! ... !$p_{n}$! = e
\end{lstlisting}
Hierbei ist $f$ der Funktionsname, $p_{1}, ..., p_{n}$ sind formale Parameter und $e$ ist der Funktionsrumpf. Mögliche Rumpfausdrücke sind unter anderem Zahlen, Basisoperationen wie die Addition ($1+2$), Funktionsanwendungen ($g e_{1} ... e_{m}$, wobei $e_{1} ... e_{m}$ selbst wieder Ausdrücke sind) oder bedingte Ausdrücke der Form $if b then e_{1} else e_{2}$ (wobei $b$ ein boolescher und $e_{1},e_{2}$ einfache Ausdrücke sind).
\\
...
Während es in Haskell vermieden werden sollte, beim Pattern Matching Regeln mit überlappenden linken Regelseiten zu definieren, ist dies in Curry zulässig. Auf diese Weise kann man nicht-deterministische Funktionen, die mehrere Lösungen liefern, definieren.
\\
Das einfachste Beispiel für eine solche nicht-deterministische Funktion ist der \lstinline|(?)|-Operator, der die Wahl zwischen zwei Möglichkeiten repräsentiert. Statt überlappender Regeln kann man auch diesen Operator verwenden, wenn für eine Regel mehrere Möglichkeiten auf der rechten Regelseite definiert werden sollen (vergleiche \lstinline|insert|-Implementierung).
\\
Die Funktion \lstinline|insert| fügt ein Element an einer beliebigen Stelle in einer Liste ein. So liefert der Aufruf \lstinline|insert 42 [1,2,3]| die folgende Lösungsmenge \lstinline|{[42,1,2,3],[1,42,2,3],[1,2,42,3],[1,2,3,42]}|. Mit Hilfe dieser Funktion lässt sich dann als weiteres Beispiel sehr leicht die Permutation von Listen implementieren.
\begin{lstlisting}[language=Haskell]
(?) :: a -> a -> a
x ? _ = x
_ ? y = y
...
path x y = edge x =:= y
path x y = edge x =:= z & path z y where z free 
\end{lstlisting}
...
\begin{enumerate}
\item \textbf{Residuation:} Die Auswertung eines (Teil-)Ausdrucks mit freien Variablen wird verzögert bis die freie Variable (z.B durch Auswertung anderer Teilausdrücke) an einen Wert gebunden wurde.
...
\subsection{KiCS2}
\label{section_kics2}
Dieser Abschnitt befasst sich mit der KiCS2-Curry-Implementierung, die im Zuge dieser Arbeit von mir um eine Schnittstelle zur Integration von Constraint-Solvern erweitert wurde. Die Vorstellung dieses Curry-Compilers beschränkt sich dabei im Wesentlichen auf die Konzepte, die für die Entwicklung dieser Erweiterung von Bedeutung sind. 
\\
Auf der \href{http://www-ps.informatik.uni-kiel.de/kics2}{KiCS2-Homepage} des Lehrstuhls kann man mehrere Artikel \textcolor{red}{$<$hier Verweis auf Quellen$>$} mit weiterführenden Informationen finden sowie die aktuelle KiCS2-Distribution herunterladen.
\\
\\
Wie bereits erwähnt, ist KiCS2 (Kiel Curry System Version 2) ein Compiler für die funktional-logische Sprache Curry, der derzeit am Lehrstuhl für Programmiersprachen und Übersetzerkonstruktion der Christian-Albrechts-Universität zu Kiel von Michael Hanus und seinen Mitarbeitern entwickelt wird.
\\
...
not _              = Fail
\end{lstlisting}
Die bisherige Implementierung des Nicht-Determinismus in Haskell hat allerdings noch einige Schwächen: Verwendet man nicht-deterministische Werte als Argumente, so kann dies zu einer semantischen Mehrdeutigkeit führen. So liefert beispielsweise der Ausdruck \lstinline|xorSelf aHaskellBool| mit der unten angegebenen Implementierung von \lstinline|xorSelf| das folgende Ergebnis: \lstinline|Choice (Choice False True) (Choice True False)|. Das bedeutet der Ausdruck liefert die Ergebnisse \lstinline|False|, \lstinline|True|, \lstinline|True| und \lstinline|False| und damit auch das nicht-gewollte Resultat \lstinline|True|.
\begin{lstlisting}[language=Haskell]
xorSelf :: Bool -> Bool
xorSelf x = xor x x
...
\end{lstlisting}
Nun liefert der Ausdruck \lstinline|xorSelf aHaskellBool| das folgende Ergebnis \lstinline|Choice 1 (Choice 1 False True) (Choice 1 True False)| (wobei die \lstinline|ID| für \lstinline|aHaskellBool| 1 sei). Somit ist sichergestellt, dass bei konsistenter Auswahl eines Wertes in allen Vorkommen einer \lstinline|Choice| (also beispielsweise immer Auswahl des linken Werts) tatsächlich nur noch der erwartete Ergebniswert \lstinline|False| herauskommen kann.
\\
\\
Curry als funktional-logische Sprache bietet auch die Möglichkeit, freie Variablen in Ausdrücken und Programmen zu verwenden. Diese freien Variablen ersetzt KiCS2 durch nicht-deterministische Generatoren in Haskell. Ein solcher Generator erzeugt dann lazy alle möglichen Werte des jeweiligen Variablentyps. Bereitgestellt wird dieser Generator von der Typklasse \lstinline|Generable|, die im Folgenden zusammen mit einer (leicht vereinfachten) Beispielinstanz für \lstinline|Bool| aufgeführt wird (In der Typklasse \lstinline|NonDet| werden alle nicht-deterministischen Typen zusammengefasst, das heißt, alle Typen, die die Erweiterung um die \lstinline|Choice|- bzw. \lstinline|Fail|-Konstruktoren unterstützen):
\begin{lstlisting}[language=Haskell]
...
Zur Optimierung und um zu gewährleisten, dass auch wirklich nur nicht-deterministische Funktionen einen \lstinline|IDSupply| als zusätzliches Argument erhalten, führt der KiCS2-Compiler vor der Übersetzung eine Determinismus-Analyse durch: Führt die zu übersetzende Curry-Funktion weder direkt noch indirekt nicht-deterministische Werte ein, so kann auf das zusätzliche Argument verzichtet werden und der für sie erzeugte Code entspricht inetwa dem funktionalen Pendant dieser Funktion.
...
instance Generable [Bool] where
  generate s = Choice (thisID s) [] (boolVar (leftSupply s) 
                                      : boolListVar (rightSupply s))
\end{lstlisting}
...
data Decision = ChooseLeft | ChooseRight | BindTo ID | NoDecision
\end{lstlisting}
Ein Bindungsconstraint ist also ein Paar bestehend aus einer Variablen-ID und der Bindungsentscheidung für diese freie Variable. Eine freie Variable kann an den linken bzw. rechten Pfad einer \lstinline|Choice| oder mittels \lstinline|BindTo| an eine andere freie Variable gebunden werden. Mit \lstinline|NoDecision| wird der Fall abgebildet, dass für die Variable bislang noch keine Bindungsentscheidung getroffen wurde.
\\
...
\\
Über die beiden Funktionen \lstinline|lookupDecision| und \lstinline|setDecision| kann man auf gespeicherte Entscheidungen zugreifen bzw. neue Entscheidungen für eine \lstinline|Choice| unter dessen \lstinline|ID| eintragen
\begin{lstlisting}[language=Haskell]
...
\label{section_mcp}
Das Monadic Constraint Programming Framework (kurz: MCP) - entwickelt von Tom Schrijvers, Peter Stuckey, Phil Wadler und Pieter Wuille - ist ein Framework zur Modellierung und Lösung von Constraint-Problemen in Haskell. Dieses Kapitel liefert nur einen knappen Überblick über die wichtigsten Features des MCP-Frameworks. Des Weiteren werden die von mir verwendeten Schnittstellen vorgestellt. Spezielle Implementierungsdetails sowie weitere Informationen kann man in zahlreichen Artikeln der Entwickler nachlesen \textcolor{red}{$<$hier Verweis auf Quellen$>$}.
\\
...
Für Finite Domain Constraints gibt es eine zusätzliche Abstraktionsschicht im MCP-Framework: Während im generischen Solver-Interface die verwendeten Constraints als Parameter übergeben werden, ist dies beim speziellen FDSolver-Interface nicht nötig. Hier ist von vornherein festgelegt, dass nur FD-Constraints verwendet werden. Abstrahiert wird nur über den konkreten Finite Domain Solver und die von ihm verwendeten Techniken zur Constraint-Propagierung.
Somit definiert das MCP-Framework eine gemeinsame Constraint-Modellierungssprache für alle Finite Domain Solver, was die Entwicklung von Solver-unabhängigen Modell-Transformationen und -Abstraktionen ermöglicht.
\\
...
\item \lstinline|BoolExpr t c b|: Zur Repräsentation von booleschen Ausdrücken (entspricht: FD-Constraints)
\end{itemize}
...
type ModelBool = BoolExpr ModelIntTerm ModelColTerm ModelBoolTerm
...
forall, forany    :: ColExpr t c b -> (Expr t c b -> BoolExpr t c b) 
                     -> BoolExpr t c b
\end{lstlisting}
...
\end{lstlisting}
Weitere Informationen findet man in den wissenschaftlichen Artikeln der Entwickler, zum Beispiel über die Integration der Gecode Solver Bibliothek in das MCP-Framework oder über die Implementierung der primitiven Suchstrategien und Such-Transformer \textcolor{red}{$<$hier Verweis auf Quellen$>$})). Wer das Framework selbst ausprobieren möchte, findet das entsprechende Package in der \href{http://hackage.haskell.org/package/monadiccp}{HackageDB}.


% EVENTUELL WEGLASSEN ??

Wie weiter oben erwähnt, ist mit der vorgestellten Modellierungssprache für FD-Constraints nur noch eine Abstraktion der FD-Solver nötig. Hierfür stellt das MCP-Framework die folgende Typklasse als Interface zur Verfügung:
\begin{lstlisting}[language=Haskell]
class Term s (FDTerm s) => FDSolver s where
  type FDIntTerm s :: *
  type FDBoolTerm s :: *
  type FDColTerm s :: *
  compile_constraint :: BoolExpr (FDIntTerm s) (FDColTerm s) (FDBoolTerm s) -> Tree s Bool
\end{lstlisting}
Bei der Implementierung eines neuen FDSolvers müssen somit einerseits Terme-Typen für Integer-, Listen- und boolesche Ausdrücke definiert und andererseits eine Funktion angegeben werden, die ein Constraint aus der MCP-Modellierungssprache in ein Baummodell für den jeweiligen Solver übersetzt. Die FDSolver-Typklasse ermöglich es nun einen generischen Solver \lstinline|FDInstance s| zu definieren, der die Transformation von einem generischen Constraint-Modell in ein Solver-spezifisches Modell für alle FD-Solver (= Solver, die die Typklasse FDSolver instanziieren) einkapselt. Das heißt, \lstinline|FDInstance s| ist dann ein Solver im Sinne der oben vorgestellten Solver-Typklasse, der \lstinline|BoolExpr (FDIntTerm s) (FDColTerm s) (FDBoolTerm s)| als Constraints verwendet:
\begin{lstlisting}[language=Haskell]
newtype FDSolver s => FDInstance s a = FDInstance { unFDInstance :: s a }

instance FDSolver s => Solver (FDInstance s) where
  type Constraint = BoolExpr (FDIntTerm s) (FDColTerm s) (FDBoolTerm s)

  add c = FDInstance $ untree $ compile_constraint c
\end{lstlisting}
