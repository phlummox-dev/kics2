...
\end{itemize}
Typen, die diese Typklasse implementieren, müssen außerdem eine \lstinline|Show|- und \lstinline|Typeable|-Instanz besitzen. Ersteres hängt damit zusammen, dass es möglich sein soll, die "'eingepacken"' Constraints auf der Kommandozeile auszugeben (z.B. zum Debuggen). Die \lstinline|Typeable|-Instanz ist hingegen für die Realisierung der Default-Implementierung von \lstinline|unwrapCs| erforderlich. Für alle drei Funktionen werden nämlich Default-Implementierungen bereitgestellt.
...
class (Typeable c, Show c) => WrappableConstraint c where
  ...
  updateVars = return
\end{lstlisting}
Zum "'Einpacken"' eines Constraints wird in der Default-Implementierung einfach der Konstruktor \lstinline|CWrapper| aufgerufen. "'Ausgepackt"' wird ein Constraint mit Hilfe der Funktion \lstinline|cast| aus \emph{Data.Typeable}. Lässt sich der gegebene Wert in den angegebenen Zieltyp casten, so liefert \lstinline|cast| den Zielwert eingepackt in einem \lstinline|Just| zurück. Andernfalls wird \lstinline|Nothing| zurückgegeben. Damit man \lstinline|cast| anwenden kann, ist es notwendig, dass Typen, die \lstinline|WrappableConstraint| implementieren, auch eine \lstinline|Typeable|-Instanz zur Verfügung stellen.
\\
Nach dem gleichen Prinzip wird \lstinline|cast| in \cite{Marlow:2006:EDH:1159842.1159854} verwendet, um eine bestimmte Exception aus einem allgemeinen Exception-Wert abzufangen.
\\
Die \lstinline|updateVars|-Funktion wird in der Default-Implementierung einfach durch \lstinline|return| realisiert. Das heißt, per Default werden die Variablenbindungen nicht aktualisiert. Diese Realisierung sollte man bei der Instanziierung von \lstinline|WrappableConstraint| also gegebenenfalls überschreiben.
\par
Um den Constraint-Wrapper in \lstinline|Guard|-Ausdrücken zum Durchreichen von Constraints durch die Implementierung einsetzen zu können, muss nun noch der \lstinline|Constraints|-Typ entsprechend angepasst werden. Man ersetzt nun den speziell für FD-Constraints eingeführten Konstruktor \lstinline|FDConstr| (vergleiche Code-Listing 13 \textcolor{red}{$<$hier endgültige Listingnr eintragen$>$}) durch einen Konstruktor für \lstinline|WrappedConstraint|s:
\begin{lstlisting}[language=Haskell,caption=Anpassung des Datentyps \lstinline|Constraints|]
data Constraints
  = ...
  | WrappedConstr [WrappedConstraint]
\end{lstlisting}
Diese Erweiterung ermöglicht es, viele verschiedene Constraint-Typen, die die Typklasse \lstinline|WrappableConstraint| implementieren, mit Hilfe von \lstinline|Guard|-Ausdrücken in einer heterogenen Liste vom Typ \lstinline|[WrappedConstraint]| durch die Implementierung durchzureichen. Der Vorteil dieser Realisierung gegenüber der ursprünglichen ist, dass man bei der Einführung weiterer Constraint-Typen in KiCS2 den \lstinline|Constraints|-Typ nun nicht mehr erweitern oder anpassen muss. Statt für jede weitere Constraint-Art einen neuen \lstinline|Constraints|-Konstruktor zu definieren, kann man diese über das \lstinline|WrappableConstraint|-Interface in KiCS2 integrieren.
\par
...
\end{lstlisting}
Zur automatischen Erzeugung der erforderlichen \lstinline|Typeable|-Instanz für \lstinline|FDConstraint| verwendet man die Haskell-Spracherweiterung \emph{derive data typeable}.
\\
...
external_d_C_prim_domain vs l u _ = 
  ...
  in Guard_C_Success (WrappedConstr [c]) C_Success
\end{lstlisting}
Die Erzeugung der \lstinline|FDConstraint|-Konstruktorterme bleibt dabei unverändert (vergleiche Code-Listings 21 und 22 \textcolor{red}{$<$hier endgültige Listingnr eintragen$>$}), aber diese Terme werden jetzt durch Aufruf von \lstinline|wrapCs| in einen Constraint-Wrapper eingepackt und damit wird dann ein neuer \lstinline|Guard|-Ausdruck konstruiert.
\par
...
\end{lstlisting}
Mit dieser Funktion ist man in der Lage - wie auch schon in Kapitel 4.1.3 für FD-Constraints beschrieben - alle in einem Pfad des KiCS2-Suchbaums vorkommenden \lstinline|WrappedConstraint|s einzusammeln. Dies ist erforderlich, damit man beim Aufruf eines spezifischen Constraint-Solvers alle nötigen Constraint-Informationen an diesen weitergeben kann.
\par
...
\\
Um später eine Liste von Constraint-Solvern definieren zu können, mit denen man nacheinander versucht, alle Constraints in der heterogenen Liste zu lösen, führt man den folgenden Datentyp ein:
...
\end{lstlisting}
Wie schon bei \lstinline|WrappedConstraint| ermöglicht es der Einsatz der Haskellspracherweiterung \emph{existential quantified type} in der Definition von \lstinline|Solver m a|, Solver für verschiedene Constraint-Typen in diesen einzelnen Typ zu packen. Der einzige Konstruktor dieses Typs, \lstinline|Solver|, erwartet eine Funktion als Argument, die ihrerseits eine Liste von "'einpackbaren"' Constraints sowie einen nicht-deterministischen Ausdruck entgegennimmt und einen neuen nicht-deterministischen Ausdruck eingekapselt in der \lstinline|Store|-Monade zurückgibt.
\\
...
\\
Da \lstinline|filterCs| die Funktion \lstinline|unwrapCs| aufruf, die ihrerseits mit Hilfe von \lstinline|cast| aus \emph{Data.Typeable} implementiert wurde, ist es notwendig, den Typ für die herauszufilternden Constraints anzugeben. Aus diesem Grund sind die Typannotationen oben bei der Definition der \lstinline|Solver|-Funktionen erforderlich.
\par
Mit dieser Filterfunktion und der Liste der unterstützten Constraint-Solver kann man nun eine Funktion angeben, die versucht sämliche "'eingepackten"' Constraints in der heterogenen Liste zu lösen, indem sie diese nacheinander nach Constraints für die einzelnen Solver durchsucht.
...
\\
Sind nach dem Ausprobieren aller unterstützten Constraint-Solver trotzdem noch ungelöste Constraints übrig, so wird eine entsprechende Fehlermeldung zurückgegeben.
\\
Solange die Liste der Solver nicht leer ist, versucht \lstinline|solveAll| durch Aufruf von \lstinline|filterCs| die für den Solver passenden Constraints aus der heterogenen Liste herauszusuchen. Diese Suche kann zu vier verschiedenen Ergebnissen führen:
\begin{enumerate}
...
\item Die Liste des gesuchten Constraint-Typs ist leer, die heterogene Liste nicht: In diesem Fall wird durch einen rekursiven Aufruf von \lstinline|solveAll| versucht, die übrigen Constraints mit dem nächsten Constraint-Solver aus der Liste zu lösen.
...
\begin{lstlisting}[language=Haskell,caption=Lösung aller "'eingepackten"' Constraints während der KiCS2-Tiefensuche]
  ...
  ...
\end{lstlisting}
Wie man sieht, wird \lstinline|solveAll| mit den \lstinline|WrappedConstraint|s \lstinline|wcs| und dem durch sie beschränkten (nicht-)deterministischen Ausdruck \lstinline|e| sowie der zuvor definierten Liste der unterstützten Constraint-Solver \lstinline|solvers| aufgerufen. Der resultierende nicht-deterministische Ausdruck wird mit Hilfe des Tiefensuchealgorithmus weiter ausgewertet.
\par
...
\\
Auf den ersten Blick erscheint es durchaus sinnvoll, die Curry-Bindungsconstraints auch in den Constraint-Wrapper einzupacken, da dann wirklich alle Constraint-Typen gleich behandelt werden könnten. 
\\
Möglich wäre dies, wenn man den Datentyp für die Curry-Bindungsconstraints in seiner ursprünglichen Form belässt und \lstinline|Guard|-Ausdrücke den folgenden Typ (im folgenden beispielhaft angegeben für \lstinline|C_Bool|) hätten:
...
\end{lstlisting}
Das bedeutet anstelle des Typs \lstinline|Constraints| würde jeder \lstinline|Guard|-Ausdruck eine Liste von \lstinline|WrappedConstraint|s enthalten. Auf diese Weise könnte man das \lstinline|WrappableConstraint|-Interface auch für die Curry-Bindungsconstraints implementieren.
\begin{lstlisting}[language=Haskell,caption=Alternativer Typ für \lstinline|Guard|-Ausdrücke]
data Constraints
...
\end{lstlisting}
Und müsste dann nur noch bei der Erzeugung von entsprechenden \lstinline|Guard|-Ausdrücken die Curry-Bindungsconstraints in ein \lstinline|WrappedConstraint| "'einpacken"' sowie den KiCS2-internen Solver für diese Constraints über die Funktion \lstinline|solveAll| ansteuerbar machen.
\\
Diese Lösung hätte den Vorteil, dass man bei der Implementierung der KiCS2-Suchfunktionen die Constraints aller \lstinline|Guard|-Ausdrücke durch \lstinline|solveAll| lösen lassen könnte.
\\
Problematisch wird das Wrapping der Curry-Bindungsconstraints jedoch in den Fällen, in denen man diese Constraints mit anderen Constraint-Typen wie zum Beispiel den Finite Domain Constraints kombiniert. In der bislang vorgestellten Implementierung wurden die Curry-Bindungsconstraints in solchen Fällen im KiCS2-Auswertungsbaum "'nach oben"' sortiert. Auf diese Weise konnten sie vor den FD-Constraints gelöst werden und damit die Bindungen der FD-Variablen noch vor dem Aufruf eines Solvers entsprechend aktualisiert werden.
\\
Dieses Problem ließe sich jedoch lösen, indem man den Solver für die Curry-Bindungsconstraints beim Aufruf von \lstinline|solveAll| als erstes ausprobiert. Auf diese Weise stünden etwaige aktuelle Bindungen dennoch rechtzeitig vor Aufruf der FD-Solver zur Verfügung. 
\\
...
\par
Behandelt man die Curry-Bindungsconstraints nun wie alle anderen Constraints und packt diese in den Constraint-Wrapper, so werden diese beim Einsammeln der \lstinline|WrappedConstraint|s durch die Funktion \lstinline|searchWrappedCs| zusammen mit den übrigen Constraints "'nach unten"' im KiCS2-Auswertungsbaum sortiert. Dies hat allerdings auch zur Folge, dass die Bindungsconstraints für \lstinline|NarrowedChoices| "'nach unten"' verschoben werden und damit auch unterhalb des zugehörigen \lstinline|NarrowedChoices|-Knoten im Auswertungsbaum platziert werden. Die Konsequenz ist, dass Berechnung von Ausdrücken, die FD-Constraints mit freien Variablen als Listenargumente mit Unifikationsausdrücken über diesen freien Variablen kombinieren, nicht mehr terminieren. Da der \lstinline|Guard|-Ausdruck mit den Bindungsconstraints im KiCS2-Auswertungsbaum "'unterhalb"' der \lstinline|NarrowedChoice|-Knoten liegt, werden die Bindungsconstraints nicht mehr vor der Auswertung der \lstinline|NarrowedChoice|-Knoten gelöst, so dass statt der durch die Bindungsentscheidung vorgegebenen Pfade sämtliche Pfade der \lstinline|NarrowedChoices| ausgewertet werden. Und deshalb terminiert die Berechnung in solchen Fällen nicht mehr.
\\
Das folgende Beispiel (einschließlich der verschiedenen Auswertungsbäume) verdeutlicht diese Problematik noch einmal:
\par
Lösen ließe sich dieses Problem nur, indem man die Curry-Bindungsconstraints beim Einsammeln von den übrigen \lstinline|WrappedConstraint|s trennt. Dafür müsste man diese allerdings aufwendig mittels \lstinline|filterCs| herausfilter. Da damit das ursprüngliche Ziel, alle Constraint-Typen gleich zu behandeln, ad absurdum geführt würde, erscheint es sinnvoller, die Curry-Bindungsconstraints von vornherein anders zu behandeln als die übrigen Constraint-Typen und sie daher nicht in den Constraint-Wrapper zu packen.
\clearpage
